...
# up to 40 pa H2 in Cord-Ruwisch 1998

def writephrq(prefix, temperature, \
              totc, totw, tots, headspace, ph, cace, fe2, 
              ffe3, fbuffer, \
              fglucose, fsom1, fsom2, fsom3, \
              fferb, fmega, fmegh, glucose_stoich, o2scalar):  
  ...
  prefix_solution = """
DATABASE ../database/redox_ph.dat
...
"""
  ...
  om_gram = fbuffer * totc * molecular_weight_c
  surf    = 34.17e3
  ha      = om_gram * 7.10E-04
  hb      = om_gram * 3.55E-04
  hc      = om_gram * 1.18E-04
  ...
  kferak = 3.89e-6  #0.014 h-1 Holmes et al. 2013, 0.04-0.09 h-1 Esteve-Nunez et al 2005; 0.088, 0.023 h-1 Cord-Ruwisch 1998 
  ofile.write('    -parms  %10.3E 1.2E-5 7.0 1.0E-6\n\n'%(kferak*tmf))
  
  kferhk = 7.78e-6 
  ofile.write('  FeRHk\n')
  ofile.write('    -formula Ferrous -114.7648 H -109.7648 H2O -13.0 Ferric 114.7648 HCO3 5.0 Hzero2 67.3824\n')
  ofile.write('    -parms  %10.3E 1.0E-6 7.0 1.0E-6\n\n'%(kferhk*tmf))

  kdecay = 5.787e-7 # Rittman and McCarty 2001 0.05 d-1
  ofile.write('  FeRd\n')
  ...
  ofile.write('  MeGAk\n')
  kmegak = 3.470e-6    # 0.3 1/d Rittmann and McCarty 2001 p169 table 3.1
  ...
  ofile.write('  MeGHk\n')
  kmeghk = 5.79e-6    # 0.5 1/d Rittmann and McCarty 2001 p169 table 3.1
...
USER_PUNCH
  -start
  -heading Time(d) pH P(atm) CO2(g) CH4(g) H2(g) CO2(a) CH4(a) H2(a) Ac Glucose Fulvic WEOC Fe(2) Fe(3) Fe(OH)3a SOM1 SOM2 SOM3 SOM4 FeRb MeGAb MeGHb Ac- Fe+2 Fe+3 H2 FeRA FeRH MeGA MeGH
...
"""
  ...
  ofile.write('  80 PUNCH equi(\'redoxFe(OH)3(a)\')/%10.6f*1000.0\n'%(totw))
  ofile.write('  90 PUNCH kin(\'SOM1\')/%10.6f*1000.0 kin(\'SOM2\')/%10.6f*1000.0 kin(\'SOM3\')/%10.6f*1000.0 kin(\'SOM4\')/%10.6f*1000.0\n'%(totw, totw, totw, totw))
  ofile.write(' 100 PUNCH kin(\'FeRb\')/%10.6f*1000.0 kin(\'MeGAb\')/%10.6f*1000.0 kin(\'MeGHb\')/%10.6f*1000.0\n'%(totw, totw, totw))
  ofile.write(' 110 PUNCH mol(\'Acetate-\')*1000.0 mol(\'Ferrous+2\')*1000.0 mol(\'Ferric+3\')*1000.0 mol(\'Hzero2\')*1000.0\n')

  output = """
 120 PUNCH SI('FeRA') SI('FeRH') SI('MeGA') SI('MeGH')
  -end
END
"""
  ofile.write(output)
